...
public class AccumulatedAggregationScheduler {
    ...
    public void aggregationMonthlyUserAccumulation() {
        try {
            ...
            LocalDateTime endOfMonth = currentYearMonth.atEndOfMonth().plusDays(1).atStartOfDay().minusNanos(1);
    ...
    private void processMemberAggregation(Member member, String month, LocalDateTime startOfMonth, LocalDateTime endOfMonth) {
    ...
    private long calculateTotalDepositTime(Member member, LocalDateTime startOfMonth, LocalDateTime endOfMonth) {
        try {
