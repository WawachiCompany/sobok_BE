...
public class TodoService {
  ...
  public ResponseEntity<?> startTodo(Member member, Long todoId) {
    try {
      ...
      if (isFirstTodo) {
        ...
        todoLog.setRoutineLog(currentRoutineLog);
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("message", "할 일 시작에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> endTodo(Long todoLogId, Long duration) {
    try {
      ...
      if (isLastTodo) {
        RoutineLog routineLog = routineLogRepository.findByRoutineAndIsCompleted(todo.getRoutine(),
            false).orElseThrow(
            () -> new IllegalArgumentException("해당 루틴의 진행 중인 로그가 존재하지 않습니다."));
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("message", "할 일 완료에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> getTodayTodos() {
    List<Todo> todos = todoRepository.findByMemberAndDay(memberService.getMember(),
